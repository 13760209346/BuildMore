...
//	private static const font13:Array = ['ar'];
	...
	public static function initializeLanguageList():void {
		...
		languages = WYBuilder.app.server.getLanguageList();
	...
	public static function setLanguage(lang:String):void {
		...
		setFontsFor(lang);
		if ('en' == lang){
			WYBuilder.app.translationChanged(); // there is no .po file English
		}else {
			var data:Object = WYBuilder.app.server.getPOFile(lang);
			if (data) {
				dictionary = data;
				checkBlockTranslations();
				setFontsFor(lang);
				WYBuilder.app.extensionManager.parseAllTranslators();
			}
			setTimeout(WYBuilder.app.translationChanged, 0);
//			WYBuilder.app.translationChanged();
		}
		langChangedSignal.dispatchEvent(new Event(Event.CHANGE));

		WYBuilder.app.server.setSelectedLang(lang);
	}
	...
	private static function fontSizeMenu():void {
		...
		m.showOnStage(WYBuilder.app.stage);
	}
...
//		for each (var entry:Array in Specs.commands) checkBlockSpec(entry[0]);
	...
	private static function checkBlockSpec(spec:String):void {
		...
		if (!argsMatch(extractArgs(spec), extractArgs(translatedSpec))) {
			WYBuilder.app.log('Block argument mismatch:');
			WYBuilder.app.log('    ' + spec);
			WYBuilder.app.log('    ' + translatedSpec);
