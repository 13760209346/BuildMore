package {
	...
	import cc.wuyuankeji.mbot.lookandfeel.MyLookAndFeel;
	import cc.wuyuankeji.mbot.ui.parts.TopSystemMenu;
	import cc.wuyuankeji.mbot.uiwidgets.errorreport.ErrorReportFrame;
	import cc.wuyuankeji.mbot.util.AppTitleMgr;
	import cc.wuyuankeji.mbot.util.PopupUtil;
	import cc.wuyuankeji.menu.MenuBuilder;
	import cc.wuyuankeji.updater.AppUpdater;
	import cc.wuyuankeji.util.FileUtil;
	import cc.wuyuankeji.util.FlashSprite;
	import cc.wuyuankeji.util.InvokeMgr;
	...
	[SWF(frameRate="30")]
	public class WYBuilder extends Sprite {
		// Version
		...
		public static var app:WYBuilder; // static reference to the app, used for debugging
		...
		private function initStage(evt:Event):void{
			...
			var extensionsPath:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu");
		...
		private function openAutoProjectFile():void
		{
			...
			if(autoProjectFile.exists)
			{
				function reductionFile(value:int):void
				{
					...
					{
						WYBuilder.app.runtime.selectedProjectFile(autoProjectFile,deleteAutoProjectFile);
					}
					else
					{
		...
		private function initExtension():void{
			...
			SerialManager.sharedManager().setWYBuilder(this);
			HIDManager.sharedManager().setWYBuilder(this);
		}
		...
		public function track(msg:String):void{
			ga.trackPageview(
				(ApplicationManager.sharedManager().isCatVersion?"/myh/":"/") + WYBuilder.versionString + msg
			);
		}
		...
		public function getMediaLibrary(app:WYBuilder, type:String, whenDone:Function):MediaLibrary {
			return new MediaLibrary(app, type, whenDone);
		}
		
		public function getMediaPane(app:WYBuilder, type:String):MediaPane {
		...
		private function onExiting(evt:Event):void{
			...
			WYBuilder.app.gh.mouseUp(new MouseEvent(MouseEvent.MOUSE_UP));
		...
		private function setSmallStageMode(flag:Boolean):void {
			...
			stagePane.updateCostume();
			//this.tabsPart.soundsTab.visible = !stageIsArduino;	//add by dragon 2019.4.21
			this.tabsPart.soundsTab.visible = false;		//add by dragon 2019.4.21
		...
		public function openMicrosoftCognitiveSetting(msg:String):void{
			var dialogBox:DialogBox = new DialogBox( function():void{
				...
				WYBuilder.app.track("/OxfordAi/setting/save");
			}); 
			...
			WYBuilder.app.track("/OxfordAi/setting/open");
		}
		// -----------------------------
		// Project Reverting
		//------------------------------
		...
		private function preDoRevert():void {
			revertUndo = new ProjectIO(WYBuilder.app).encodeProjectAsZipFile(stagePane);
			doRevert();
		}
