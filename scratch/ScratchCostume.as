...
public class ScratchCostume {
	...
	public function setBitmapData(bm:BitmapData, centerX:int, centerY:int):void {
		...
		rotationCenterX = centerX;
		rotationCenterY = centerY;
		if (WYBuilder.app && WYBuilder.app.viewedObj() && (WYBuilder.app.viewedObj().currentCostume() == this)) {
			WYBuilder.app.viewedObj().updateCostume();
			WYBuilder.app.refreshImageTab(true);
		}
	}
	...
	public function setSVGData(data:ByteArray, computeCenter:Boolean, fromEditor:Boolean = true):void {
		...
		function refreshAfterImagesLoaded():void {
			...
			if (WYBuilder.app && WYBuilder.app.viewedObj() && (WYBuilder.app.viewedObj().currentCostume() == thisC)) {
				WYBuilder.app.viewedObj().updateCostume();
				WYBuilder.app.refreshImageTab(fromEditor);
			}
			svgLoading = false;
		}
...
//trace('found bounds: '+new Rectangle(minX, minY, maxX - minX, maxY - minY));
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		var oldQuality:String = WYBuilder.app.stage.quality;
		WYBuilder.app.stage.quality = StageQuality.LOW;
		bm.draw(dispObj, m);
		WYBuilder.app.stage.quality = oldQuality;
