...
public class ScratchStage extends ScratchObj {
	...
	public function spriteNamed(spriteName:String):ScratchSprite {
		...
		var app:WYBuilder = WYBuilder.app;
	...
	public function spritesAndClonesNamed(spriteName:String):Array {
		...
		var app:WYBuilder = parent as WYBuilder;
		if (app != null) {
...
//		if(WYBuilder.app.isIn3D) return WYBuilder.app.render3D.getUIContainer();
		return this;
	}

	override protected function updateImage():void {
		super.updateImage();
//		if(WYBuilder.app.isIn3D)
//			WYBuilder.app.render3D.getUIContainer().transform.matrix = transform.matrix.clone();
...
//			if(WYBuilder.app.isIn3D) WYBuilder.app.render3D.updateRender(videoImage);
		}
...
//			WYBuilder.app.render3D.updateRender(penLayer);
...
//		if(!WYBuilder.app.render3D || s.width < 1 || s.height < 1) return null;
		return null;
/*
		...
		var bmd:BitmapData = WYBuilder.app.render3D.getRenderedChild(s, b.width*s.scaleX, b.height*s.scaleY, for_carry);
	...
	public function setVideoState(newState:String):void {
		...
		WYBuilder.app.libraryPart.showVideoButton();
	...
	public function clearPenStrokes():void {
		...
		penActivity = false;
//		if(WYBuilder.app.isIn3D) WYBuilder.app.render3D.updateRender(penLayer);
	}
...
//		if(WYBuilder.app.isIn3D) WYBuilder.app.render3D.updateFilters(spr, effects);
	}
	...
	public function getBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		...
		if(WYBuilder.app.isIn3D) {
			var b:Rectangle = s.currentCostume().bitmap ? s.img.getChildAt(0).getBounds(s) : s.getVisibleBounds(s);
			bm1 = WYBuilder.app.render3D.getOtherRenderedChildren(s, 1);
			//mask = 0x80F8F8F0;
		}
		else {*/
			// OLD code here
...
//		if(WYBuilder.app.isIn3D) WYBuilder.app.render3D.updateRender(dispObj, renderID, renderOpts);
	}
...
//		if(WYBuilder.app.isIn3D) WYBuilder.app.render3D.getRender(bm);
//		else
		bm.draw(this);
	...
	public function updateInfo():void {
		...
		if (WYBuilder.app.projectID != '') info.projectID = WYBuilder.app.projectID;
		info.videoOn = isVideoOn();
		info.swfVersion = WYBuilder.versionString;

		delete info.loadInProgress;
		if (WYBuilder.app.loadInProgress) info.loadInProgress = true; // log flag for debugging

		if (this == WYBuilder.app.stagePane) {
			// If this is the active stage pane, record the current extensions.
			var extensionsToSave:Array = WYBuilder.app.extensionManager.extensionsToSave();
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (obj is ScratchSprite) {
				...
				WYBuilder.app.selectSprite(obj);
			...
			if (!(obj is ScratchSprite) || WYBuilder.app.editMode) WYBuilder.app.setSaveNeeded();
			return true;
		}
		WYBuilder.app.setSaveNeeded();
		if ((obj is MediaInfo) && obj.fromBackpack) {
			...
			var app:WYBuilder = root as WYBuilder;
			// Add sprites
			if (obj.mysprite) {
