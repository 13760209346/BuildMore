...
//		var b:Rectangle = (!WYBuilder.app.render3D || currentCostume().bitmap) ? img.getChildAt(0).getBounds(this) : getVisibleBounds(this);
		...
		if(WYBuilder.app.isIn3D) {
			...
			updateEffects();
			var bm:BitmapData = WYBuilder.app.render3D.getRenderedChild(this, b.width*scaleX, b.height*scaleY);
...
//		}
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		...
		//if (tool == 'help') WYBuilder.app.showTip('scratchUI');
	}

	private function growSprite():void { setSize(getSize() + 5); WYBuilder.app.updatePalette() }
	private function shrinkSprite():void { setSize(getSize() - 5); WYBuilder.app.updatePalette() }
...
//			}
	...
	private function saveToLocalFile():void {
		function success():void {
			WYBuilder.app.log('sprite saved to file: ' + file.name);
		}
		var zipData:ByteArray = new ProjectIO(WYBuilder.app).encodeSpriteAsZipFile(copyToShare());
	...
	private function bubbleRect():Rectangle {
		...
		// compute bounds
		var p:Point = WYBuilder.app.stagePane.globalToLocal(localToGlobal(new Point(0, 0)));
		if (cachedBounds == null) bitmap(); // computes cached bounds
