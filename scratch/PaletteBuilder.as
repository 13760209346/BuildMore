...
public class PaletteBuilder {
	...
	public function PaletteBuilder(app:WYBuilder) {
		this.app = app;
	}
...
//		var catName:String = Specs.categories[selectedCategory][1];
	...
	{
		if(WYBuilder.app.viewedObj() is ScratchSprite){
			return category;
		}
		switch(category){
	...
	static private function canShowInArduinoMode(spec:Array):Boolean
	{
		...
		if(WYBuilder.app.stageIsArduino && categoryId == Specs.controlCategory){
			switch(spec[3]){
				case "stopScripts":
				case "whenCloned":
				case "createCloneOf":
	...
	private function addExtensionSeparator(ext:ScratchExtension):void {
		...
		indicator.addEventListener(MouseEvent.CLICK, function(e:Event):void {WYBuilder.app.showTip('extensions');}, false, 0, true);
