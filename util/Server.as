...
import cc.wuyuankeji.util.CsvReader;
import cc.wuyuankeji.util.Excel;
...
/*
		...
		function errorHandler(err:ErrorEvent):void {
			...
			loader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, errorHandler);
			loader.removeEventListener(IOErrorEvent.IO_ERROR, errorHandler);
			WYBuilder.app.logMessage('Failed server request for '+url);
			whenDone(null);
		}
	...
	*/	
	...
	public function getAsset(md5:String):ByteArray
	{
		...
		file = File.applicationStorageDirectory.resolvePath("yunTu/media/" + md5);
		if(file.exists){
			return FileUtil.ReadBytes(file);
		}
		return null;
...
//			return null;
//		}
//		return fetchAsset('app-storage:/yunTu/media/' + md5);
	}

	public function getMediaLibrary():String
	{
		return fetchAsset('app-storage:/yunTu/media/mediaLibrary.json').toString();
	}
	...
	public function getPOFile(lang:String):Object
	{
		...
		return obj[lang];
		/*
		var file:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/locale");
		var bytes:ByteArray;
		if(file.exists){
	...
	{
		var file:File = File.applicationStorageDirectory.resolvePath("yunTu/locale/locale.xlsx");
		if(!file.exists){
