...
public class ProjectIO {
	...
	protected var app:WYBuilder;
	protected var images:Array = [];
	protected var sounds:Array = [];

	public function ProjectIO(app:WYBuilder):void {
		this.app = app;
	}
	...
	private function decodeFromZipFile(zipData:ByteArray):ScratchObj {
		...
		}else if(jsonData.indexOf("BuildDouble")>-1){//add by dragon
			if(!WYBuilder.app.extensionManager.checkExtensionSelected("BuildDouble")){
				WYBuilder.app.extensionManager.onSelectExtension("BuildDouble");
			}
		}else if(jsonData.indexOf("Makeblock")>-1){
			if(!WYBuilder.app.extensionManager.checkExtensionSelected("Makeblock")){
				WYBuilder.app.extensionManager.onSelectExtension("Makeblock");
			}
		}else if(jsonData.indexOf("Arduino.")>-1){
			if(!WYBuilder.app.extensionManager.checkExtensionSelected("Arduino")){
				WYBuilder.app.extensionManager.onSelectExtension("Arduino");
			}
		}
