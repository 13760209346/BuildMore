...
public class ListWatcher extends Sprite {
	...
	private function importLines(lines:Array):void {
		...
		DialogBox.ask(
			...
			'1', WYBuilder.app.stage, gotColumn);
	}
...
//		WYBuilder.app.interp.redraw();
	...
	private function addItem(b:IconButton = null):void {
		...
		if ((root is WYBuilder) && !(root as WYBuilder).editMode) return;
...
//		if (limitedCloudView &&
//			WYBuilder.app.isLoggedIn() && WYBuilder.app.editMode &&
//			(WYBuilder.app.projectOwner == WYBuilder.app.userName)) {
//				limitedCloudView = false; // only project owner can view cloud list contents
//		}
		var isEditable:Boolean = WYBuilder.app.editMode && !limitedView;
		updateElementCount();
		removeAllCells();
		visibleCells = [];
		visibleCellNums = [];
