...
package watchers {
	...
	import cc.wuyuankeji.util.StringChecker;
...
//			runtime.interp.redraw();
	...
	private function specForCmd():String {
		...
		if(i > -1) {
			var spec:Array = WYBuilder.app.extensionManager.specForCmd(cmd);
	...
	private function getValue(runtime:ScratchRuntime):* {
		...
		if(index >= 0){
			...
			var ext:ScratchExtension = WYBuilder.app.extensionManager.extensionByName(extName);
...
/*
		if(cmd.indexOf('.') > -1) {
			...
			if(spec) {
				...
				return WYBuilder.app.interp.evalCmd(block);
			}
		}
*/
		return "unknown: " + cmd;
	...
	public function doubleClick(evt:MouseEvent):void {
		if (!WYBuilder.app.editMode) return;
	...
	public function menu(evt:MouseEvent):Menu {
		function handleMenu(item:int):void {
			...
			if (item == 10) {
				visible = false;
				WYBuilder.app.updatePalette(false);
			}
		}
		if (!WYBuilder.app.editMode) return null;
...
//			m.addItem("text", 4);
	...
	private function mouseDown(evt:MouseEvent):void {
		...
		if (p.y > 20) WYBuilder(root).gh.setDragClient(this, evt);
	}
