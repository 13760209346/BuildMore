...
//		WYBuilder.app.runtime.checkForGraphicEffects();
	}
...
/*
	public function showHelp():void {
		...
		if((i = op.indexOf('.')) > -1) {
			...
			if(WYBuilder.app.extensionManager.isInternal(extName))
				WYBuilder.app.showTip('ext:'+extName);
			else
				DialogBox.notify('Help Missing', 'There is no documentation available for experimental extension "'+extName+'".', WYBuilder.app.stage);
		}
		else {
			WYBuilder.app.showTip(op);
		}
	}
*/
	public function duplicateStack(deltaX:Number, deltaY:Number):void {
		if (isProcDef()) return; // don't duplicate procedure definition
		var forStage:Boolean = WYBuilder.app.viewedObj() && WYBuilder.app.viewedObj().isStage;
		var newStack:Block = BlockIO.stringToStack(BlockIO.stackToString(this), forStage);
		newStack.x = x + deltaX;
		newStack.y = y + deltaY;
		parent.addChild(newStack);
		WYBuilder.app.gh.grabOnMouseUp(newStack);
	}
...
//		var top:Block = topBlock();
//		// TODO: Remove any waiting reporter data in the WYBuilder.app.extensionManager
//		if (parent is Block) Block(parent).removeBlock(this);
...
//		if (top != this) x += top.width + 5;
//		removeComments();
//		WYBuilder.app.runtime.recordForUndelete(this, x, y, 0, WYBuilder.app.viewedObj());
//		WYBuilder.app.scriptsPane.saveScripts();
//		WYBuilder.app.runtime.checkForGraphicEffects();
//		return false;
	}
	private function removeComments():void{
		var comments:Array = attachedCommentsIn(WYBuilder.app.scriptsPane);
		...
		WYBuilder.app.scriptsPane.fixCommentLayout();
	}
	...
	public function objToGrab(evt:MouseEvent):Block {
		if (isEmbeddedParameter() || isInPalette()) return duplicate(false, WYBuilder.app.viewedObj() is ScratchStage);
		return this;
	}
	...
	public function click(evt:MouseEvent):void {
		...
		if(topBlock().op.indexOf("runArduino")>-1){
			if(!WYBuilder.app.stageIsArduino){
				WYBuilder.app.changeToArduinoMode();
			}
			return;
		}
		WYBuilder.app.runtime.interp.toggleThread(topBlock(), WYBuilder.app.viewedObj(), 1);
	}
