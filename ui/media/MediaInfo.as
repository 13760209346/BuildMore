...
public class MediaInfo extends Sprite {
	...
	public function objToGrab(evt:MouseEvent):* {
		var result:MediaInfo = WYBuilder.app.createMediaInfo({
			type: objType,
			name: objName,
			md5: md5
		});
		if (mycostume) result = WYBuilder.app.createMediaInfo(mycostume, owner);
		if (mysound) result = WYBuilder.app.createMediaInfo(mysound, owner);
		if (mysprite) result = WYBuilder.app.createMediaInfo(mysprite);
		if (scripts) result = WYBuilder.app.createMediaInfo(scripts);
	...
	protected function duplicateMe():void {
		if (owner) {
			if (mycostume) WYBuilder.app.addCostume(mycostume.duplicate());
			if (mysound) WYBuilder.app.addSound(mysound.duplicate());
		}
	}

	protected function deleteMe(ignore:* = null):void {
		if (owner) {
			WYBuilder.app.runtime.recordForUndelete(this, 0, 0, 0, owner);
			if (mycostume) {
				owner.deleteCostume(mycostume);
				WYBuilder.app.refreshImageTab(false);
			}
			if (mysound) {
				owner.deleteSound(mysound);
				WYBuilder.app.refreshSoundTab()
			}
		}
	}
