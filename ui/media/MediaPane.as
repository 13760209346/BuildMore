...
public class MediaPane extends ScrollFrameContents {
	...
	public function MediaPane(app:WYBuilder, type:String):void {
		this.app = app;
		isSound = (type == 'sounds');
		refresh();
	}
	...
	private function costumeItems():Array {
		...
		for each (var c:ScratchCostume in viewedObj.costumes) {
			result.push(WYBuilder.app.createMediaInfo(c, viewedObj));
		}
		return result;
	}
	...
	private function soundItems():Array {
		...
		for each (var snd:ScratchSound in viewedObj.sounds) {
			result.push(WYBuilder.app.createMediaInfo(snd, viewedObj));
		}
		return result;
	}
