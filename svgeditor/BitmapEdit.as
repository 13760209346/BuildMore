...
public class BitmapEdit extends ImageEdit {
	...
	public function BitmapEdit(app:WYBuilder, imagesPart:ImagesPart) {
	...
	private function bakeIntoBitmap(doClear:Boolean = true):void {
		...
		if (bm && (content.numChildren > 0)) {
			...
			if (!WYBuilder.app.runtime.shiftIsDown) stage.quality = StageQuality.LOW;
			for (var i:int = 0; i < content.numChildren; i++) {
				var el:DisplayObject = content.getChildAt(i) as DisplayObject;
				var textEl:SVGTextField = el as SVGTextField;
				
				if (textEl && !WYBuilder.app.runtime.shiftIsDown) {
					...
					var c:uint = 0xFF000000 | textEl.textColor;
					clearOffscreenBM();
					offscreenBM.draw(textEl);
					// force pixels above threshold to be text color, alpha 1.0
					offscreenBM.threshold(offscreenBM, offscreenBM.rect, new Point(0, 0),'>', threshold, c, 0xFF000000, false);
					// force pixels below threshold to be transparent
					offscreenBM.threshold(offscreenBM, offscreenBM.rect, new Point(0, 0),'<=', threshold, 0, 0xFF000000, false);
					// copy result into work bitmap
					bm.draw(offscreenBM, m, null, null, null, true);
				} else {
