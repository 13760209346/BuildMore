package extensions
{
	...
	import cc.wuyuankeji.mbot.util.PopupUtil;
	import cc.wuyuankeji.mbot.util.StringUtil;
...
//			addEventListener(EVENT_COMPILE_DONE,uploadHex,false);
		...
		public function clearTempFiles():void
		{
			...
			PopupUtil.showConfirm(Translator.map("Restart yunTu?"),WYBuilder.app.restart);
		}
		
		public function setScratch(scratch:WYBuilder):void{
			_scratch = scratch;
		}
		/*
...
//					}
		...
		private function getCodeBlock(blk:Object,params:Array=null):CodeBlock{
			...
			else if(blk[0]=="writeVariable:")
			{
				...
				var objs:Array = WYBuilder.app.extensionManager.specForCmd(blk[0]);
				if(objs!=null){
					var obj:Object = objs[objs.length-1];
					obj = obj[obj.length-1];
					if(typeof obj == "object"){
						var ext:ScratchExtension = WYBuilder.app.extensionManager.extensionByName(blk[0].split(".")[0]);
...
//				}
		...
		private function parseModules(blks:Object):void{
			...
			for(var i:int;i<blks.length;i++){
				var b:Object = blks[i];
				var objs:Array = WYBuilder.app.extensionManager.specForCmd(blks[0]);
				if(objs!=null){
					var obj:Object = objs[objs.length-1];
					obj = obj[obj.length-1];
		...
		private function parseCodeBlocks(blks:Object):Boolean{
			...
			for(var i:int;i<blks.length;i++){
				...
				if(op.indexOf("runArduino")>-1){
					...
					var objs:Array = WYBuilder.app.extensionManager.specForCmd(op);
					var ext:ScratchExtension = WYBuilder.app.extensionManager.extensionByName(op.split(".")[0]);
					if(ext!=null){
						if(srcDocuments.indexOf(ext.srcPath)==-1){
							srcDocuments.push(ext.srcPath);
						}
...
//			var srcdir:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/libraries/"+_extSrcPath+"/src");
...
//			}
		...
		*/
		...
		private function get projectDocumentName():String{
			...
			var pName:String = WYBuilder.app.projectName().split(" ").join("").split("(").join("").split(")").join("");
			//用正则表达式来过滤非法字符
		...
		public function buildAll(ccode:String):String
		{
			...
			if(arduinoInstallPath==""){
				...
				function onDownload():void{
					flash.net.navigateToURL(new URLRequest("http://learn.wuyuankeji.cc/learning-arduino/"));
					dialog.cancel();
				}
				dialog.addButton("Cancel",onCancel);
				dialog.addButton("Set Path",onSetPath);
				dialog.addButton("Download",onDownload);
				dialog.showOnStage(WYBuilder.app.stage);
...
//				}
		...
		private function buildArduinoLib(buildDir:File):void{
			...
			if(_currentDevice=="uno" || _currentDevice=="leonardo"){
				listArduinoLib(new File(arduinoInstallPath+"/libraries/wuyuankeji"));
			}
...
//			trace("currentDevice:",_currentDevice);
			if(_currentDevice=="uno"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega328p -DF_CPU=16000000L -DARDUINO=10605 -DARDUINO_AVR_UNO -DARDUINO_ARCH_AVR -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/standard -I"+path+"/libraries/Servo/src -I"+path+"/libraries/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial -I" + path+"/libraries/wuyuankeji/src"
			}else if(_currentDevice=="leonardo"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=10605 -DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR -DUSB_VID=0x2341 -DUSB_PID=0x8036 -DUSB_MANUFACTURER=\"Unknown\" -DUSB_PRODUCT=\"Arduino Leonardo\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+"/libraries/Servo/src -I"+path+"/libraries/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial -I" + path+"/libraries/wuyuankeji/src"
		...
		private function gotoNextNativeCmd(event:Event):void{
			...
			if(nativeWorkList.length>0 && compileErr==false){
				...
				WYBuilder.app.scriptsPart.appendMessage(nativeProcessStartupInfo.executable.nativePath)
				WYBuilder.app.scriptsPart.appendMessage(nativeProcessStartupInfo.arguments.toString())
				process.start(nativeProcessStartupInfo); 
		...
		{ 
			...
			WYBuilder.app.scriptsPart.appendMessage(""+(date.month+1)+"-"+date.date+" "+date.hours+":"+date.minutes+": Got: "+output); 
			*/
		}
		...
		private function onExit(event:NativeProcessExitEvent):void
		{
			...
			WYBuilder.app.scriptsPart.appendMessage(""+(date.month+1)+"-"+date.date+" "+date.hours+":"+date.minutes+": Process exited with "+event.exitCode);
			numOfSuccess++;
			if(event.exitCode > 0){
				WYBuilder.app.scriptsPart.appendMsgWithTimestamp(errorText, true);
				errorText = null;
			}
			if(compileErr == false){
				dispatchEvent(new Event(EVENT_NATIVE_DONE));
