package extensions
{
	...
	import cc.wuyuankeji.interpreter.BlockInterpreter;
	import cc.wuyuankeji.mbot.util.AppTitleMgr;
	import cc.wuyuankeji.util.UploadSizeInfo;
...
//			_avrdudeConfig = _isMacOs?"avrdude_mac.conf":"avrdude.conf";
		...
		public function setWYBuilder(yunTu:WYBuilder):void{
			_yunTu = yunTu;
		}
		...
		public function update():void{
			if(!_serial.isConnected){
				WYBuilder.app.topBarPart.setDisconnectedTitle();
				return;
			}else{
				WYBuilder.app.topBarPart.setConnectedTitle("Serial Port");
			}
		}
		...
		public function upgrade(hexFile:String=""):void{
			...
			WYBuilder.app.track("/OpenSerial/Upgrade");
			executeUpgrade();
			_hexToDownload = hexFile;
			WYBuilder.app.topBarPart.setConnectedTitle(AppTitleMgr.Uploading);
			ArduinoManager.sharedManager().isUploading = false;
		...
		public function openSource():void{
			WYBuilder.app.track("/OpenSerial/ViewSource");
			var file:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/firmware/" + getFirmwareName());
		...
		public function disconnect():void{
			currentPort = "";
			WYBuilder.app.topBarPart.setDisconnectedTitle();
//			WYBuilder.app.topBarPart.setBluetoothTitle(false);
			ArduinoManager.sharedManager().isUploading = false;
			_serial.close();
			_serial.removeEventListener(Event.CHANGE,onChanged);
		}
...
//			if(NativeProcess.isSupported) {
				...
				if(currentDevice=="leonardo"){
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_mega1280:String = (ApplicationManager.sharedManager().documents.nativePath+"/yunTu/tools/hex/mega1280.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_nano328:String = (ApplicationManager.sharedManager().documents.nativePath+"/yunTu/tools/hex/nano328.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_nano168:String = (ApplicationManager.sharedManager().documents.nativePath+"/yunTu/tools/hex/nano168.hex");//.split("\\").join("/");
...
//				process.addEventListener(IOErrorEvent.STANDARD_ERROR_IO_ERROR, onIOError);
				...
				sizeInfo.reset();
				WYBuilder.app.scriptsPart.clearInfo();
				WYBuilder.app.scriptsPart.appendMessage(nativeProcessStartupInfo.executable.nativePath + " " + v.join(" "));
...
//			}
		...
		private function getHexFilePath():String
		{
			...
			return ApplicationManager.sharedManager().documents.nativePath + "/yunTu/tools/hex/" + fileName + ".hex";
		}
		/*
...
//			_dialog.setText(Translator.map('Executing')+" ... "+_upgradeBytesLoaded+"%");
		...
		*/
		...
		private function onStandardOutputData(event:ProgressEvent):void
		{
			...
			WYBuilder.app.scriptsPart.appendRawMessage(msg);
		}
		...
		*/
		public function executeUpgrade():void {
			...
			_dialog.showOnStage(_yunTu.stage);
		}
