...
import cc.wuyuankeji.interpreter.RemoteCallMgr;
...
public class ExtensionManager {
	...
	public function ExtensionManager(app:WYBuilder) {
		this.app = app;
...
//		if(isCommonExt(name)){
	...
	{
		var ext:Object = WYBuilder.app.extensionManager.findExtensionByName(extName);
	...
	private function refreshList():void{
		_extensionList = [];
		if(ApplicationManager.sharedManager().documents.resolvePath("yunTu/libraries/").exists){
			var docs:Array =  ApplicationManager.sharedManager().documents.resolvePath("yunTu/libraries/").getDirectoryListing();
	...
	*/
	public function copyLocalExtensionFiles():void{
		...
		for each(var sf:File in srcFile.getDirectoryListing()){
			var tf:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/libraries/"+sf.name);
			if(!tf.exists){
				sf.copyTo(tf,true);
			}else{
...
//		}
	...
	private function copyFirmwareAndHex():void{
		...
		var tf:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/firmware/mblock_firmware/");
		srcFile.copyTo(tf,true);
		var botFile:File = File.applicationDirectory.resolvePath("firmware/mbot_firmware/");
		var bottf:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/firmware/mbot_firmware/");
		botFile.copyTo(bottf,true);
		var hexFile:File = File.applicationDirectory.resolvePath("tools/hex/");
		var htf:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/tools/hex/");
		hexFile.copyTo(htf,true);
		var localsFile:File = File.applicationDirectory.resolvePath("locale/");
		var ltf:File = ApplicationManager.sharedManager().documents.resolvePath("yunTu/locale/");
	...
	static private function copyDir(dirName:String, destDirName:String=null):void
	{
		...
		var toFile:File = File.applicationStorageDirectory.resolvePath("yunTu").resolvePath(destDirName || dirName);
...
//								WYBuilder.app.runtime.exitRequest();
							//}
						}
					}
				}
...
//		parseAllTranslators();
		WYBuilder.app.translationChanged();
		WYBuilder.app.updatePalette();
...
//			WYBuilder.app.runtime.enterRequest();
...
//		if (ext.port > 0) {
//			
//		} else if(WYBuilder.app.jsEnabled) {
...
//			b.nextID.push(ext.nextID);
			...
			var objs:Array = WYBuilder.app.extensionManager.specForCmd(ext.name+"."+op);
