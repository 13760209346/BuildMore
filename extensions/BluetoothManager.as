package extensions
{
	...
	public class BluetoothManager
	{
		...
		public function discover():void{
			if(_bt){
				...
				if(!_bt.isDiscovering){
					WYBuilder.app.track("/OpenBluetooth");
					...
					d.showOnStage(WYBuilder.app.stage);
		...
		public function open(port:String):Boolean{
			...
			function checkName():void{
				if(_bt.connected){
					...
					_isBusy = false;
					addBluetoothHistory();
					WYBuilder.app.topBarPart.setConnectedTitle("Bluetooth");
		...
		private function onDiscoverChanged(evt:Event):void{
			...
			_list = str.split(",");
			//_list.push("asdf","fda","666","7777","asdfasdfasdf","谭启亮","mblock","wuyuankeji","airblock","flash builder","谭启亮","mblock","wuyuankeji","airblock","flash builder","end")
			...
			for(var i:uint=0;i<_list.length;i++){
				for(var j:* in _dialogboxDiscover){
					...
					d.x = (WYBuilder.app.stage.stageWidth-d.width)/2<0?0:(WYBuilder.app.stage.stageWidth-d.width)/2;
					d.y = (WYBuilder.app.stage.stageHeight-d.height)/2<0?0:(WYBuilder.app.stage.stageHeight-d.height)/2;
				}
			}
