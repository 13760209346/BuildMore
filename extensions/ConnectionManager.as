	...
	public class ConnectionManager extends EventDispatcher
	{
		...
		public function onConnect(name:String):void{
			switch(name){
				...
				case "reset_program":{
					SerialManager.sharedManager().upgrade(ApplicationManager.sharedManager().documents.resolvePath("yunTu/tools/hex/mbot_reset.hex").nativePath);
					break;
				}
				...
				case "driver":{
					WYBuilder.app.track("/OpenSerial/InstallDriver");
					var fileDriver:File;
...
//						navigateToURL(new URLRequest("https://github.com/Makeblock-official/Makeblock-USB-Driver"));
				...
				default:{
					...
					if(isConnectCmd) {	// collect data on what type of board it connected to
						...
						WYBuilder.app.track("/ConnectBoard/"+boardName);
		...
		public function onClose(port:String):void{
			...
			if(!SerialDevice.sharedDevice().connected){
				...
				if(SerialManager.sharedManager().isConnected||HIDManager.sharedManager().isConnected||BluetoothManager.sharedManager().isConnected){
					...
					WYBuilder.app.topBarPart.setConnectedTitle("Network");
				}
			}
			BlockInterpreter.Instance.stopAllThreads();
		...
		public function onReceived(bytes:ByteArray):void{
			_bytes = bytes;
			WYBuilder.app.scriptsPart.onSerialDataReceived(bytes);
